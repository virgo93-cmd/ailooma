...
- Article, page, and author inventories are queried from the published WordPress REST API with the shared `wordpress` cache tag and a 300-second data revalidation. The existing WordPress webhook invalidates that tag. A normal post publish/update/draft therefore changes sitemap membership or `lastmod` without requiring a Vercel redeploy. CMS-backed sitemaps render on request so CMS timeouts do not fail static-site generation. The category sitemap uses the five canonical category paths in the site’s declared navigation, not the intermittently timing-out CMS category listing endpoint. Hub routes come from the existing hub registry.
...
- `npm run build` after the adjustment: **PASS**, Next.js 16.3.4; 91 static/dynamic route outputs generated, including all five child sitemap routes and the index.
- Vercel deployment and public sitemap/redirect/canonical/schema verification: **PENDING**.
...
To be completed only after the source commit is pushed and Vercel publishes it. Record observed child URL counts, XML/HTTP status, duplicate and disallowed URL counts, `lastmod` comparison, representative page metadata/render checks, and the deployment URL/commit here.
